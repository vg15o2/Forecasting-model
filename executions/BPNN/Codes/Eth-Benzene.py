...
# Extract the 'From Date' and 'Eth-Benzene (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['Eth-Benzene (ug/m3)'].values
...
target_train = train_data['Eth-Benzene (ug/m3)'].values
target_test = test_data['Eth-Benzene (ug/m3)'].values
...
results.to_excel(f'BPNNEth-Benzeneforecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of Eth-Benzene')
plt.legend()
plt.show()
fig.savefig('BPNNEth-Benzene.png')
