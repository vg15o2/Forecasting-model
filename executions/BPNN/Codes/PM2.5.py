...
# Extract the 'From Date' and 'PM2.5 (ug/m3) (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['PM2.5 (ug/m3)'].values

# Filter the data for the desired date range
train_data = data[(dates >= '23-03-2015') & (dates <= '2020')]
test_data = data[(dates >= '2021') & (dates <= '2025')]

# Extract the target variable from the filtered data
target_train = train_data['PM2.5 (ug/m3)'].values
target_test = test_data['PM2.5 (ug/m3)'].values
...
model = MLPRegressor(hidden_layer_sizes=(50, 50), activation='relu', solver='lbfgs', random_state=42)
...
results.to_excel(f'BPNNPM2.5forecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of PM2.5')
plt.legend()
plt.show()
fig.savefig('BPNNPM2.5.png')
