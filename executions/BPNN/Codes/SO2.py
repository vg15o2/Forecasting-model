...
# Extract the 'From Date' and 'SO2 (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['SO2 (ug/m3)'].values
...
target_train = train_data['SO2 (ug/m3)'].values
target_test = test_data['SO2 (ug/m3)'].values
...
results.to_excel(f'BPNNSO2forecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of SO2')
plt.legend()
plt.show()
fig.savefig('BPNNSO2.png')
