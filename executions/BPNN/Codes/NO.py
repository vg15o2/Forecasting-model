...
# Extract the 'From Date' and 'NO (ug/m3) (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['NO (ug/m3)'].values
...
target_train = train_data['NO (ug/m3)'].values
target_test = test_data['NO (ug/m3)'].values
...
results.to_excel(f'BPNNNOforecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of NO')
plt.legend()
plt.show()
fig.savefig('BPNNNO.png')
