...
# Extract the 'From Date' and 'Ozone (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['Ozone (ug/m3)'].values
...
target_train = train_data['Ozone (ug/m3)'].values
target_test = test_data['Ozone (ug/m3)'].values
...
results.to_excel(f'BPNNOzoneforecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of Ozone')
plt.legend()
plt.show()
fig.savefig('BPNNOzone.png')
