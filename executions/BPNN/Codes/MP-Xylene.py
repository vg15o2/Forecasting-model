...
# Extract the 'From Date' and 'MP-Xylene (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['MP-Xylene (ug/m3)'].values
...
target_train = train_data['MP-Xylene (ug/m3)'].values
target_test = test_data['MP-Xylene (ug/m3)'].values
...
results.to_excel(f'BPNNMP-Xyleneforecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of MP-Xylene')
plt.legend()
plt.show()
fig.savefig('BPNNMP-Xylene.png')
