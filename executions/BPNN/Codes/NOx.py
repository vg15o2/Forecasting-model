...
# Extract the 'From Date' and 'NO2 (ug/m3) (ug/m3)' columns
dates = pd.to_datetime(data['From Date'])
target = data['NOx (ppb)'].values
...
target_train = train_data['NOx (ppb)'].values
target_test = test_data['NOx (ppb)'].values
...
results.to_excel(f'BPNNNOxforecast_results_fold_{fold}.xlsx', index=False)
...
plt.xlabel('Time')
plt.ylabel('Value')
plt.title('Comparison of Forecast and Actual Values of NOx')
plt.legend()
plt.show()
fig.savefig('BPNNNOx.png')
